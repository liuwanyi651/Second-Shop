<template>
  <div>
  <!--详情页导航返回部分 使用NavBar导航栏组件 left-arrow 是否显示左侧箭头-->
    <div>
        <van-nav-bar left-text="返回" title="订单结算" left-arrow @click-left="onClickLeft" fixed />
    </div>
    <div class="mt40">
    <!--ContactCard 联系人卡片--->
    <van-contact-card type="edit" :name="currentContact.name" :tel="currentContact.tel" @click="onEdit"/>
    <div >
        <div class="bbs flex">
            <div class="pic">
                <img :src="this.list.img" alt="">
            </div>
            <div class="cr fs14 lih30 ml20">
                <div class="mt20">
                {{this.list.name}}
                </div>
                <div class="mt10 fw-b">
                ￥{{this.list.price}}
                </div>     
            </div>  
            <div class=" mt50 fs14 ml10">
                x{{this.list.value}}
            </div>   
        </div>
    </div>
    <div>
        <van-tabbar >
            <van-tabbar-item></van-tabbar-item>
            <van-tabbar-item></van-tabbar-item>
            <van-tabbar-item >
              <span class="fs14 cr">合计:￥{{(this.list.price * this.list.value).toFixed(2)}}</span>
            </van-tabbar-item>
            <van-tabbar-item >
              <van-button type="danger" @click="submitOrder">提交订单</van-button>
            </van-tabbar-item>
        </van-tabbar>
    </div>
    </div>
  </div>
</template>

<script>
export default {
  name: '',
  props:{},
  data () {
    return {
         currentContact: {
            name: '张三',
            tel: '13000000000', 
      },
       orderpay:{},
            list:'',
            total:'',
    }
  },
  components: {},
  methods: {
       //顶部导航返回 点击返回到上一页
        onClickLeft() {
            window.history.go(-1)
        },
         onEdit(){
      this.$toast('编辑');
    },
     getOrder(){
        // console.log(this.$route); 
        let orderpay = this.$route.query.data || this.$route.params.data
        // console.log(orderpay);//打印 query 获取的东西
        this.list = JSON.parse(orderpay) //把字符串转换成对象 赋给 this.list
        console.log( this.list);
     },
     submitOrder(){
       let submit = {
         address:'',
            tel:'',
            orderId:[this.list.id],
            totalPrice:this.list.price * this.list.value,
            idDirect:true,
            count:this.list.value,
            img:this.list.img
       };
       this.$api.orderShop(submit).then(res =>{
         this.$toast.success(res.msg)
       }).catch(err =>{
         console.log(err);
       })
     },

  },
  mounted() {
          this.getOrder()  
          //拿到购物车 商品 和 价格的值
          this.orderProduct = JSON.parse(localStorage.getItem(carGoods))                     
          this.orderTotal = JSON.parse(localStorage.getItem(carPrice))                     
  },
  computed: {
     
  },
  watch: {}
}
</script>

<style lang="scss" scoped>
.pic{
    width: 100px;
    height: 100px;
    border: 1px solid #EEE;
    img{
        width: 100%;
    }
}

</style>