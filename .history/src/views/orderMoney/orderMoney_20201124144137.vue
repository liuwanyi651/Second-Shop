   ...
   <div v-if="address===null" @click="add" class="flex j-center a-center">
    ...
    <div>
        <van-tabbar >
            ...
            <van-tabbar-item >
              <van-button type="danger" @click="pay" >提交订单</van-button>
            </van-tabbar-item>
        </van-tabbar>
    </div>
    </div>
...
export default {
  ...
  methods: {
      ...
      pay(){
        let arr = [];
        this.list.map((item)=>{
          arr.push(item.cid) //给每一项 增加cid的属性
        })
        let payInfo = {
          ...
          idDirect:this.list[0].idDirect,  // 第一项为true
          count:this.list[0].count
        };
        this.$api.orderShop(payInfo).then(res=>{
          console.log(res);
