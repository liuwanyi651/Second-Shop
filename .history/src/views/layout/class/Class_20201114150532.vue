    ...
    <div class="flex">
        ...
        <div>
            ...
            <van-sidebar v-model="activeKey">
                ...
                <van-sidebar-item v-for="(item,index) in goodsCategoryName" :key="index" :title="item.mallCategoryName" @click="getClassify(item.bxMallSubDto)" fixed="true" />
            </van-sidebar>
        </div>
        <!--上面-->
        <div class="top h">
            <!--使用Tab标签页组件 上边分类 热带水果什么的-->
            <van-tabs v-model="active" @click="aboveChange" class="pd8 " fixed=true>
                ...
                <van-tab v-for="(item,index) in classQuery" :key="index" :title="item.mallSubName" :name="item.mallSubId" swipeable>
                    ...
                    <div v-for="(item,index) in goodsData" :key="index">
                        <!--@click="goto(item)" 跳转到详情页-->
                        <div class="picIimg  flex bs m10td" @click="goto(item)">
                            <div class="pic">
