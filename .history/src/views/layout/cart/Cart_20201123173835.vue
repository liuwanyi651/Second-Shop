          ...
          <div>
            ...
            <div class="flex">
              ...
              <div style="width: 55px">
                <van-button type="danger" size="small"  @click="goOrder()">去结算</van-button>
              </div>
            </div>
          </div>
        </div>
...
// count:商品数量 (购物车购物传入第一个商品的count)
    ...
    goOrder(){
       this.list.map(item =>{
        ...
        let carGoods = this.arr
        let carPrice = this.mallPrice.toFixed(2)
        localStorage.setItem("carGood",JSON.stringify(carGood))
        localStorage.setItem("carPrice",JSON.stringify(carPrice))
       }) 
       this.$router.push('/ordeMoneyr')
    }
  },
  mounted() {
    this.getCar();
