...
<div>
    ...
    <div >
        <van-goods-action class="z">
            ...
            <!--使用Popup 弹出层 点立即购买 会弹出相应的数量-->
            <van-popup v-model="show" position="bottom" closeable  close-icon="close" :style="{ height: '32%' }" >
                <div class="box bbs flex">
                    <div class="picture">
                        <img :src="this.list.image" alt="">
                    </div>
                    <div>
                        <div class="c5 fs14 lih50">
                        {{this.list.name}}
                        </div>
                        <div class="cr">
                        ￥{{this.list.present_price}}
                        </div>
                    </div>
                </div>
                <div class="bbs pd10 flex">
                    <div>
                        <div class="fs12 pd5">
                        购买数量：
                        </div>
                        <div class="c179 fs12 pd5">
                        剩余{{this.list.amount}} 件<span class="cr fs12 pdl10">每人限购50件</span>
                        </div>
                    </div>
                    <div class="ml50 m10">
                        <!--Stepper 步进器-->
                        <van-stepper v-model="value" max="50" @change="add(value)"/>
                    </div>
                </div>
                <div>
                    <van-button type="danger" class="w-max" @click="goMoney(list)">立即购买</van-button>
                </div>
            </van-popup>
        </van-goods-action>
    </div>
...
export default {
        ...
        getDetail() {
            ...
            this.$api.getGoods(this.id).then(res => {
                if (res.code === 200) {
                    ...
                    res.goods.goodsOne.cid = this.list.id
                    res.goods.goodsOne.count=this.value
                    res.goods.goodsOne.mallPrice = this.list.present_price
                    res.goods.goodsOne.idDirect = true
    ...
    goMoney(list){
        let goods = this.arr
        let price = this.list.present_price*this.value
        localStorage.setItem("goods",JSON.stringify(goods))
        localStorage.setItem("price",JSON.stringify(price))
        this.$router.path("/orderMoney")
    },
    // 步进器增加
    add(value){
        this.value=value
        this.arr[0].count = value
    }
    },
    mounted() {
        //调用这个方法
