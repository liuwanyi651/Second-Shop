...
<div>
    ...
    <van-grid :border="false" :column-num="2" class="">
        <van-grid-item v-for="(item,index) in hotGoods" :key="index" class="hot" @click="goto(item.goodsId)">
            <!--实参-->
            <!--绑定一个事件 路由传参-->
            <!--相应的图片-->
            <div>
...
export default {
    ...
    methods: {
        ...
        //路由传参 
        //这个id是形参 拿来接收 随便什么字母都可以 
        goto(id) {
            // console.log(id);  //从首页点击热销商品 获取到id值 fb0f913950944b66a97ae262ad14609a
            //push还可以传入一个对象 对象可以传入path属性 也可以传入name属性 name是路由配置的name 不是组件内部的name
            this.$router.push({
                name: 'detail', //路由跳转到detail详情页面
                //query传参  query又是一个对象
                query: {
                    getId: id //getId 是自己定义的名字
                }
            })
        }
    },
