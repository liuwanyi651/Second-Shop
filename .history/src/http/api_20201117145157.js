...
export default {
    ...
    //10. 取消收藏(post) /cancelCollection 参数: id:商品的id
    cancel(id){
    ...
    // id: 用户id
    // nickname: 昵称
    changeSaveUser({gender,year,month,day,id,nickname}){
    ...
    // image: []
    discuss({id,rate,content,anonymou,_id,order_id, image}){
        return http.post('/goodsOne/comment',{
            id,rate,content,anonymou,_id,order_id, image
        })
    },
    //18.获取登录注册默认验证码(get)
    getCode(){
    ...
    // areaCode:地区代码
    // id:修改地址时候要传id
    addShoptAddress({name,tel,address,isDefault,province,city,county,addressDetail,areaCode,id}){
